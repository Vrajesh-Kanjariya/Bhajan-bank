...
class NiyamViewModel {
  late SelectNiyamPage niyamPage;
  ...
  List<SubCategoryDataInfo> subCategoryInfoList = [];
  ...
  bool sadgranthAnusthsan = false;
  bool mukhPath = false;
  bool sadgranthVachan = false;
  ...
  Future<void> getCategoryData() async {
    ...
    if (categoryResponse != null) {
      ...
      await getSubCategoryData(categoryResponse.info.first.id);
    }
  ...
  Future<void> getSubCategoryData(int catId) async {
    ...
    subCategoryResponse = await niyamController!
        .getSubCategoryData({'registerId': registrationId, 'memberId': membersId});
    subCategoryList = [];
    if (subCategoryResponse != null) {
      subCategoryInfoList = subCategoryResponse!.info;
      for (int i = 0; i < subCategoryInfoList.length; i++) {
        subCategoryList.addAll(subCategoryInfoList[i].subcategoryList);
      }
    }
    niyamController!.update();
  }

  void selectItemClick(int subIndex) {
    subCategoryInfoList[3].subcategoryList[subIndex].isSelect = !subCategoryInfoList[3].subcategoryList[subIndex].isSelect;
    saveNiyamHistory();
    niyamController!.update();
  }
  ...
  void getSelectedCategoryData(int index, NiyamController controller) async {
    if (index == 1) {
      ...
      //     categoryName: categoryList[index].catName, subCategoryList: subCategoryList);
    } else if (index == 2) {
      await getSubCategoryData(categoryList[index].id);
      //await getSubCategoryData(categoryList[index].id);
      // goToMukhPath(categoryName: categoryList[index].catName, subCategoryList: subCategoryList);
    } else if (index == 3) {
      await getSubCategoryData(categoryList[index].id);
      //await getSubCategoryData(categoryList[index].id);
      // goToSadgranthVanchan(categoryName: categoryList[index].catName, subCategoryList: subCategoryList);
    } else {
  ...
  void onTapSubCategory(int index) async {
    if (selectedIndex == 0) {
      ...
      goToChestaNiyam(subCategoryInfoList: niyamViewModel!.subCategoryInfoList[4].subcategoryList, index: index);
      getNiyamData();
    }
    niyamController!.update();
  }
